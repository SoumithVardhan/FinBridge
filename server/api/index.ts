// Complete FinBridge API for Vercel - ALL TYPESCRIPT ERRORS FIXED FINAL
...
// Initialize Supabase client - FIXED CONNECTION
...
// Health check - FIXED ALL TYPE ERRORS
app.get('/api/health', async (req, res) => {
  try {
    ...
    try {
      ...
      dbErrorMessage = (err as Error).message;
    }
    ...
    if (dbErrorMessage) {
      (healthResponse.services as any).databaseError = dbErrorMessage;
    }
...
// Register - FIXED ALL TYPE ERRORS
app.post('/api/auth/register', authLimiter, async (req, res) => {
  try {
    ...
    // Check if user already exists - FIXED ALL TYPE ERRORS
    let existingUser: { email: string; phone: string } | null = null;
    ...
    try {
      ...
      console.error('User existence check exception:', err);
    ...
    if (checkError) {
      ...
      if (process.env.NODE_ENV === 'development') {
        errorResponse.error = checkError;
        errorResponse.supabaseUrl = supabaseUrl;
        errorResponse.hasSupabaseKey = !!supabaseKey;
      }
